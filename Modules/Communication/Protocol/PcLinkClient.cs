...
// 이 시스템에서 실제로 쓰는 명령은 "CH01~07 + UNUSED + DOSTS 9개 레지스터 읽기" 하나뿐이라
// 범용 프로토콜 파서를 만들지 않고 이 고정 명령 하나만 처리하도록 최대한 단순하게 구현했다.
// 자세한 프레임 구조와 예제는 Doc/pclink protocol.md 참고.
public static class PcLinkClient
{
    public const int Port = 5000;

    private const string Station = "01"; // 국번, 전 압축기 공통(overview.md 4.2)
    private const string ReadRegisters = "0360,0361,0362,0364,0365,0366,0367,0099,1805"; // CH01~07, UNUSED, DOSTS
    ...
    public static async Task<(bool Ok, decimal[] Values, string Raw)> ReadChannelsAsync(string ipAddress, int timeoutMs = 3000)
    {
    ...
    // OK가 아니거나 체크섬이 안 맞으면 전부 실패(Ok=false)로 취급한다 — 세부 에러 코드는 구분하지 않는다(단순화 원칙).
    private static (bool Ok, decimal[] Values, string Raw) ParseResponse(byte[] buffer, int len)
    {
        ...
        // 값은 소수점 1자리 기준 10배 스케일링되어 오므로 10으로 나눈다.
        var values = fields.Skip(2)
            .Select(hex => Convert.ToInt16(hex, 16) / 10m)
            .ToArray();
