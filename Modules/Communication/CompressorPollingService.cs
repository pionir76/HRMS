...
// 1초마다: 활성 압축기 목록을 DB에서 새로 읽고 → 전부 동시에 TCP로 값을 읽어온 뒤
...
{
    private const int PollIntervalMs = 1000; // overview.md 8.1의 시스템 공통 Polling Interval과 동일
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                // 한 사이클 전체가 실패해도(예: DB 순단) 서비스 자체는 죽지 않고 다음 사이클을 계속 시도한다.
    ...
    private async Task PollOnceAsync(CancellationToken stoppingToken)
    {
        ...
        if (!testMode)
            query = query.Where(x => x.Compressor.IpAddress != null); // 테스트 모드에서는 IP 없는 압축기도 포함
        ...
        // 여기서부터는 단일 스레드로 DbContext를 순차 갱신하므로 동시성 문제가 없다.
        ...
        // 채널 → 압축기 → 장비 순으로 "가장 심각한 상태"를 집계하고 운전 여부를 판정한다.
        await EquipmentStatusAggregator.UpdateAsync(db, stoppingToken);
    }

    // -200 ~ 1200 범위로 생성한다. 기본 경보 상/하한(0~1000)을 넘나들게 해서
    // 경보발생대기/경보발생/정상복귀대기 전이가 실제로 일어나는 걸 볼 수 있다.
    private static decimal[] GenerateTestValues() =>
        [.. Enumerable.Range(0, 7).Select(_ => Math.Round((decimal)(Random.Shared.NextDouble() * 1400 - 200), 1))];
    ...
    private static async Task UpdateCurrentValuesAsync(
        AppDbContext db,
        IEnumerable<(int Id, CommunicationStatus PreviousStatus, bool Ok, decimal[] Values)> successfulResults,
        CancellationToken stoppingToken)
    {
        ...
        {
            if (r.Values.Length < 7) continue; // 방어적 체크: 정상 응답이면 항상 9개(그중 앞 7개 사용)

            for (int i = 0; i < 7; i++)
            {
                var channelNo = (ChannelNo)(i + 1); // Values[0..6] = CH01..CH07 (PcLinkClient의 레지스터 순서와 일치)
