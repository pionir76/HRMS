...
- **압축기 채널값(`value`, `ch01`~`ch07`)은 전부 TLC 원시값(raw int16)이다.** 서버는 소수점 변환을 하지 않는다 — 실제 값으로 표시하는 건 프론트가 담당한다. 채널별 소수점 자리수(dp) 정보는 아직 API로 노출되지 않는다.
...
| `alarmStatus`(장비/압축기 목록 API) | 정상, 경보발생대기, 경보발생, 정상복귀대기, 경보비활성화 (경보확인/경보해제는 없음) |
| `channelNo` | CH01, CH02, CH03, CH04, CH05, CH06, CH07 |

운전 상태는 처음부터 "운전/정지" 둘 뿐이라 enum 문자열이 아니라 **boolean `isRunning`**으로 내려간다. 트렌드 API(`/trend`)는 `alarmStatus`/`communicationStatus`도 `hasAlarm`/`isConnected`(boolean)로 대신한다 — 6번 항목 참고.
...
> 참고: 장비의 실시간 파생 상태(`isRunning`/`alarmStatus`/`communicationStatus`, 관리자가 설정하는 `status`와는 별개)는 아직 이 응답에 포함되어 있지 않다. DB(`Equipments` 테이블)에는 이미 있으니, 필요해지면 DTO에 추가하면 된다.
...
`value`는 **TLC로부터 받은 원시값(raw int16) 그대로**다. 서버는 소수점 변환을 전혀 하지 않으며, 실제 표시값으로 바꾸는 건 프론트 책임이다 (예: 위 `-123`은 실제로는 `-12.3`을 뜻함). 채널별 소수점 자리수(dp) 정보는 아직 API로 노출되지 않는다 — 현재는 백엔드 내부에서만 쓰인다.
...
`ch01`~`ch07`도 `/channels`와 마찬가지로 **원시값(raw int16)**이다. 소수점 변환은 프론트가 담당한다.
